...
async function fetchAllSupplierInvoices() {
  ...
  let cursor = null;
  for (let i = 0; i < 50; i++) {
    const path = `/supplier_invoices?limit=100${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ''}`;
    const data = await pnlFetch(path);
    all.push(...(data.items || []));
    if (!data.has_more) break;
    cursor = data.next_cursor;
...
async function fetchSupplierNames() {
  ...
  let cursor = null;
  for (let i = 0; i < 20; i++) {
    const path = `/suppliers?limit=100${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ''}`;
    const data = await pnlFetch(path);
    for (const s of data.items || []) map.set(s.id, s.name);
    if (!data.has_more) break;
    cursor = data.next_cursor;
  }
  return map;
