  ...
  if (unpaidS.count === 0 && c401.byAccount.length === 0 && duplicates.length === 0) {
    ...
    if (duplicates.length) {
      ...
      duplicates.slice(0, 3).forEach((d) => {
        out.push(`  - ${escapeMd(d.supplierName)} · ${EUR.format(d.amount)} · ${d.items.length} occurrences (${d.items.map((i) => escapeMd(i.ref)).slice(0, 3).join(', ')})`);
      });
    }
  }
...
function section3Demandes({ unpaidC, duplicates, chargeAnomalies, customerMap, includeRequests }) {
  ...
  if (duplicates.length) {
    const lines = duplicates.slice(0, 3).map((d) => `  - ${escapeMd(d.supplierName)} · ${EUR.format(d.amount)} · ${d.items.length} factures (${d.items.map((i) => escapeMd(i.ref)).join(', ')})`);
