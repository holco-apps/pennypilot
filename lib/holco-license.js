...
function writeCache(cache) {
  try {
    ...
    log.warn('license.cache.write_failed', { err: err.message });
  }
}
...
export async function assertHolcoLicense() {
  ...
  if (process.env.PENNYPILOT_BYPASS_LICENSE === '1') {
    log.warn('license.bypass_enabled', { reason: 'PENNYPILOT_BYPASS_LICENSE=1' });
    return;
  }
  ...
  try {
    ...
    if (cache && cache.key_hash === keyHash && cache.valid === true) {
      log.warn('license.registry_unreachable_using_cache', { err: err.message });
      return;
    }
    throw new Error(
